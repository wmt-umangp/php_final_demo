    ...
    if(strlen($_SESSION['alogin'])==0){
        ...
        if(isset($_POST['add_book'])){
            ...
            else
            {
                ...
                $sql="INSERT INTO  books(book_title,book_pages,book_language,book_author,book_image, book_ISBN, book_desc, book_price, book_status) VALUES('$b_title',$b_pages,'$b_lang','$b_author', '$imgnewname', '$b_isbn','$b_desc','$b_price','$bst')";
        ...
        <div class="row">
            <div class="col mt-5">
                <div class="card rounded-5 shadow-lg">
                    <h2 class="card-b_imgheader rounded-5 p-3 text-center bg-primary text-white">Add Book</h2>
                    <div class="card-body">
                        <form id="add_book_form" method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'])?>" enctype="multipart/form-data">
                            ...
                            <div class="mb-3 mt-3">
                                ...
                                <select name="b_author[]" class="form-select" id="b_author" multiple required>
                                    <option value="">Select Book's Author</option>
                                    <?php 

                                        $sql = "SELECT id, CONCAT(auth_fname,' ',auth_lname) AS auth_name from  authors ";
                                        $result=mysqli_query($conn,$sql);
                                        ...
                                        {   
                                        ?>  
                                            <option value="<?php echo htmlentities($row['auth_name']);?>"><?php echo htmlentities($row['auth_name']);?></option>
                                        <?php
                                     }} 
                                     ?> 
                                </select>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-2">
                                        <div class="form-check form-switch">
                                            <input class="form-check-input" type="checkbox" role="switch" id="b_status" name="b_status" >
                            ...
                            <button type="submit" class="btn btn-primary" id="add_book" name="add_book">Add
                                Book</button>
                            <a type="button" href="list-book.php" class="btn btn-danger" id="b_cancel" name="b_cancel">cancel</a>
                        </form>
                    </div>
                </div>
            </div>
